...
import TournamentCategoryName from '../TournamentCategoryName/TournamentCategoryName';
import TournamentCheckoutBar from '../TournamentCheckoutBar/TournamentCheckoutBar';

import TournamentCatgoryPlayerInfo from './TournamentCategoryPlayerInfo/TournamentCatgoryPlayerInfo';
...
const TournamentDraftBooking = ({
  ...
  const { tournament } = tournamentRegisteration;
  const { selectedCategories } = tournamentRegisteration;
  const [playerFormData, setPlayerFormData] = useState({});
  const isCollectPlayerData = tournament?.collectPlayerData;
  // API call to get player tournament form data
  const { data: playerTournamentFormDataResponse, isLoading: isLoadingPlayerData, error: playerDataError, refetch: refetchPlayerData } = useGetPlayerTournamentFormData(player?.id, tournament?._id, {enabled: false});

  useEffect(() => {
    if (Object.keys(tournamentRegisteration).length === 0) {
      navigate("/tournaments");
    }
  }, [tournamentRegisteration, navigate]);

  // Prefill form data when API data is loaded successfully
  useEffect(() => {
    if (playerTournamentFormDataResponse?.data?.formData && tournament?.playerFormFields) {
      const prefilledData = {};
      const formData = playerTournamentFormDataResponse.data.formData;
      
      // Map API data to form fields by exact label match
      tournament.playerFormFields.forEach((field) => {
  ...
  }, [playerTournamentFormDataResponse, tournament?.playerFormFields]);
  ...
  useEffect(() => {
    ...
    setIsPaymentSummaryTrue(booking.tournamentId !== "" && booking.playerId !== "" && isDoublesCategoriesFilledWithPartners);

    const playerFormValid = !tournament?.collectPlayerData ? true : tournament.playerFormFields.every((field) => {
          ...
          {isCreateDraftBookingError && (
            <div className="flex flex-row items-center justify-start gap-3 px-[16px] md:px-[88px] bg-f4f5ff pt-2">
              <div
                ...
                onClick={() => navigate(`/tournaments/${tournament.handle}`)}
              >
                <img
                  src={ChevronIcon}
                  alt="Back Button"
          ...
          */}
          <div className="flex flex-row justify-between items-center px-[30px] pt-9 pb-6 bg-gradient-to-b from-f4f5ff to-[#fcfdff] border-b border-f0f0f0">
            <div className="flex flex-col items-start gap-1.5 max-w-[50%]">
              <h1 className="font-author font-medium text-383838 text-2xl leading-none">{tournament.tournamentName}</h1>
              <p className="font-author font-medium text-383838 text-2xl">
                {tournament.tournamentLocation.address.city}
              </p>
            </div>
            ...
            <div className="flex flex-col items-end gap-5">
              <div className="flex flex-row gap-2">
                ...
                <p className="text-383838 opacity-70 text-sm font-general font-medium">
                  {formatDate(tournament.startDate, true, 'MMM', false)} -{' '}
                  {formatDate(tournament.endDate, true, 'MMM', false)}
                </p>
              </div>
        ...
        {tournament?.collectPlayerData && (
          <div className="block pt-10 mb-1">
            ...
            <div
              ...
              style={{
                gridTemplateColumns:
                  tournament?.playerFormFields?.length > 3
                    ? "repeat(auto-fit, minmax(280px, 1fr))"
                    : "1fr",
              }}
            >
              {tournament?.playerFormFields?.map((field, idx) => (
                <FloatingLabelInput
                  key={idx}
                  label={field.label}
          ...
          {/*
            ┌─────────────────────────────────────────────────────────────────────────────┐
            │           Payment Summary if Booking is Valid                               │
            └─────────────────────────────────────────────────────────────────────────────┘
          */}
              ...
              <div className="block mx-auto w-72">
                <div className="flex flex-row items-center justify-start gap-2 mb-5">
                  <Checkbox
                    ...
                    className="cursor-pointer block w-5 h-5 rounded border border-56b918 bg-transparent transition data-[checked]:bg-transparent"
                  >
                    {/* SVG Used Inline as needed for conditional styling */}
                    <svg
          ...
          <TournamentCheckoutBar isBookingValid={isBookingValid} createDraftBookingError={createDraftBookingError} playerFormData={playerFormData} />
        </div>
      </div>
    </main>
  );
...
export default TournamentDraftBooking;