...
const TournamentLiveInfo = (tournament) => {
  ...
  const addressInfo = {
    line1:
      tournament?.tournament?.tournamentLocation?.address?.line1 || "line 1",
    line2:
      tournament?.tournament?.tournamentLocation?.address?.line2 || "line 2",
    city: tournament?.tournament?.tournamentLocation?.address?.city || "city",
    state:
      tournament?.tournament?.tournamentLocation?.address?.state || "state",
    postalCode:
      tournament?.tournament?.tournamentLocation?.address?.postalCode ||
      "postal code",
  };
  ...
  if (skillLevel.toLowerCase() === "all levels") {
    ...
    const skillLevelIndex = tournamentSkillLevelDefaults.findIndex(
  ...
  return (
    <div
      className={`w-full bg-white px-9 md:px-20 py-10 gap-2 ${
        isLoggedIn ? "pb-10" : "pb-0"
      }`}
    >
      <div className="flex items-center justify-between gap-2 mb-2">
        <p className="text-[34px] font-author font-medium text-383838 leading-8 tracking-normal w-[60%]">
        ...
        {isLive() && (
          <div className="flex items-center gap-1 border border-56b918 rounded-2xl py-2 px-2">
            ...
            <p className="font-general font-medium text-56b918 text-[10px] md:text-xs uppercase leading-1">
              live now
            </p>
          </div>
        )}
      ...
      <div className="flex items-center gap-2 mb-2">
        <ThunderboltIcon
          ...
          iconClassName="w-4 h-4 object-cover"
        />
        <p className="font-general font-medium text-383838 text-xs md:text-sm opacity-70 capitalize">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-start gap-1 max-w-[70%]">
          ...
          <div className="w-full max-w-full">
            <p className="font-author font-medium text-sm md:text-2xl text-1c0e0e truncate">
              {tournament?.tournament?.tournamentLocation?.name}
            </p>
            <p className="font-general font-medium text-xs md:text-base text-1c0e0e opacity-50 truncate">
              {addressInfo.line1}, {addressInfo.line2}, {addressInfo.city},{" "}
      ...
      <div>
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div>
            <p className="font-general font-medium text-383838 opacity-70 text-sm md:text-base">
              Duration
            </p>
            <div className="flex items-center gap-2">
              ...
              <p className="font-medium font-general text-xs text-383838 opacity-70 md:text-sm">
                {formatDate(tournament?.tournament?.startDate)} -{" "}
                {formatDate(tournament?.tournament?.endDate)}
              </p>
            </div>
          </div>
          {whatsappLink && (
