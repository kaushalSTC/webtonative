    ...
    <div>
      <div className='w-full flex items-center justify-between border-b border-b-1c4ba3 bg-white px-9 md:px-20'>
        {TournamentLiveTabs.map((tab) => {
          return (
            <input type='button' key={tab}  value={tab} className={`${selectedTab === tab ? 'border-b-1c4ba3 border-b-2' : '' } font-general font-medium text-sm md:text-base text-383838`} onClick={() => setSelectedTab(tab)}/>
          )
        })}
      </div>
