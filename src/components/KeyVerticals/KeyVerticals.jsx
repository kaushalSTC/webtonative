...
import { AboutKeyVerticals, DottedLine } from '../../assets';
import { nanoid } from 'nanoid';

const KeyVerticals = () => {
    const { data, isLoading, error } = useGetAboutUs({ sectionName: 'keyVerticals' });
    ...
    return (
        <div className='px-5 md:px-[74px] py-[65px] bg-white'>
            <div className='mb-10 px-[15px] md:px-0 relative'>
                ...
                <p className='font-general font-medium text-sm md:text-base text-1c0e0eb3 bg-white absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 px-3'>Key Verticals</p>
            </div>

            <div>
                {sectionData?.keyVerticals.map((slide) => {
                    return (
                        <div className={`flex items-center gap-5 justify-center mb-10 ${slide.position % 2 === 0 ? 'flex-row-reverse' : 'flex-row'}`} key={nanoid()}>
                            <div className='max-w-[285px]'>
                                <img src={slide?.image || AboutKeyVerticals } alt="how-it-works" className='w-full h-auto object-cover' />
                            </div>
