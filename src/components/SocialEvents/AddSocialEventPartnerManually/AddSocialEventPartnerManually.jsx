...
import { useSendPartnerOTPMutation, useVerifyAndUpdatePartnerMutation } from "../../../hooks/SocialEventHooks";
...
const AddSocialEventPartnerManually = ({categoryId}) => {
  const { booking, event } = useSelector((state) => state.socialEventRegistration);
  ...
  const handleisOTPSent = () => {
    if (isFormValid) {
      sendPartnerOTP({
        ...
        tournamentId: event.tournamentData._id,
      });
    }
  };
  ...
  const verifyPartner = () => {
    if (isFormValid) {
      verifyAndUpdatePartner({
        ...
        tournamentId: event.tournamentData._id,
        otp: otp,
        bookingId: booking.tournamentBookingId,
...
export default AddSocialEventPartnerManually