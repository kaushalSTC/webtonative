...
import { HeaderCloseIcon } from '../../../assets';

const SocialEventGallery = ({ event }) => {
  const eventGalleryData = event.eventGallery || [];
  const [isOpen, setIsOpen] = useState(false);
  const [activeIndex, setActiveIndex] = useState(0);

  if (eventGalleryData.length <= 0) return null;
    ...
    <>
      <div className="w-full bg-white py-10 pb-8 gap-[18px] flex flex-col mt-[10px]">
        ...
        <div className="flex flex-row justify-between items-center md:gap-[10px] gap-[18px] overflow-x-auto md:pl-[80px] md:pr-[10px] pl-[30px] pr-[6px] [&::-webkit-scrollbar]:hidden">
          {eventGalleryData.map((image, index) => (
            <div 
              key={nanoid()} 
      ...
      <Popup isOpen={isOpen} className='bg-[#0000007a] inset-0 z-50 fixed grid place-items-center' handleViewOpen={handleViewOpen}>
        <div className='bg-white w-[90%] md:w-[50%] min-w-[340px] mt-13 p-6 md:p-4 rounded-lg relative shadow-2xl max-h-[80vh] overflow-auto'>
          <img src={HeaderCloseIcon} alt="close-icon" className="absolute top-2 right-2 cursor-pointer w-4 h-4" onClick={() => handleViewOpen(false)} />
          <Swiper
            modules={[Navigation]}
            navigation={true}
            className="mySwiper"
...
export default SocialEventGallery;