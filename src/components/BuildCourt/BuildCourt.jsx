...
const BuildCourt = () => {
  ...
  const getWhyChoosePickleBayDetails = async () => {
    try {
      const response = await axios.get(
        `${baseURL}/api/public/homepage-sections?section=buildCourt`
      );
  ...
  if (!homepageBannerObj || Object.keys(homepageBannerObj).length === 0 || homepageBannerObj?.DesktopBanner?.image === undefined) return null;
